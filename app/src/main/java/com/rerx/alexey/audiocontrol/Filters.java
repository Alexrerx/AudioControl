...
public class Filters {
    ...
    public static Map<Double, Double> Antialiasing(Map<Double, Double> spectrum) {
        ...
        for (int j = 0; j < spectrum.size() - 4; j++) {
            int i = j;
            double x0 = (double) keys.get(i);
            double x1 = (double) keys.get(i+1);
            double y0 = (double) values.get(i);
            double y1 = (double) values.get(i+1);

            double a = (y1 - y0)/(x1 - x0);
            double b = y0 - a*x0;

            i += 2;
            double u0 = (double) keys.get(i);
            double u1 = (double) keys.get(i+1);
            double v0 = (double) values.get(i);
            double v1 = (double) values.get(i+1);
